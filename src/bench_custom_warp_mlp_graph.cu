...
int main()
{
    ...
    // Tuned-ish defaults for T4. We'll tune later if needed.
    constexpr int BW1 = 8;
    constexpr int BW2 = 4;
    constexpr int BW3 = 1;
    ...
    // --- Benchmark: direct launches ---
    auto direct_times = bench_stream_us([&](cudaStream_t s) { infer_once(s); }, stream, warmup, iters,
                                        /*inner_reps=*/1);
    auto d = compute_percentiles(direct_times);
    print_stats("custom_warp_mlp_fp32_direct (3x kernel)", d);

    // --- Build CUDA Graph (single launch per inference) ---
    cudaGraph_t graph {};
    ...
    // --- Correctness check: graph ---
    CUDA_CHECK(cudaGraphLaunch(graphExec, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    CUDA_CHECK(cudaMemcpy(h_out.data(), d_y3, OUT_DIM * sizeof(float), cudaMemcpyDeviceToHost));
    double max_abs_err_graph = 0.0;
    for (int i = 0; i < (int)OUT_DIM; i++) {
        max_abs_err_graph = std::max(max_abs_err_graph, std::abs((double)h_out[i] - (double)y3[i]));
    }
    std::cout << "Graph correctness: max_abs_err=" << max_abs_err_graph << "\n";

    // --- Benchmark: graph launch ---
    auto graph_times =
        bench_stream_us([&](cudaStream_t s) { CUDA_CHECK(cudaGraphLaunch(graphExec, s)); }, stream, warmup, iters,
                        /*inner_reps=*/1);

    auto g = compute_percentiles(graph_times);
    print_stats("custom_warp_mlp_fp32_graph (1x cudaGraphLaunch)", g);
