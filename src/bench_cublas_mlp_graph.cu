...
int main()
{
    ...
    // --- Direct benchmark ---
    auto direct_times = bench_stream_us([&](cudaStream_t) { infer_once(); }, stream, warmup, iters,
                                        /*inner_reps=*/1);
    auto dp = compute_percentiles(direct_times);
    print_stats("cublas_mlp_fp32 (direct)", dp);
    ...
    std::cout << "Graph correctness: max_abs_err=" << max_abs_err << " (float32 CPU ref)\n";

    // --- Graph benchmark ---
    auto graph_times =
        bench_stream_us([&](cudaStream_t s) { CUDA_CHECK(cudaGraphLaunch(exec, s)); }, stream, warmup, iters,
                        /*inner_reps=*/1);
    auto gp = compute_percentiles(graph_times);
    print_stats("cublas_mlp_fp32 (CUDA Graph)", gp);
